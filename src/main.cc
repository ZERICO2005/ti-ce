...
const char * input_matrix(bool list) {
  ...
  *sptr = "";
  ...
  if (w.empty())
    ...
    msg = ((lang ? "Creer nouveau ou editer " : "Create new or edit ") + (w.size() == 1 ? w.front() : giac::gen(w, giac::_SEQ__VECT)).
      print(contextptr));
  lock_alpha();
...
std::string get_searchitem(std::string & replace) {
  ...
  std::string search;
  lock_alpha();
  int res = inputline(lang ? "EXIT ou chaine vide: annulation" : "EXIT or empty string: cancel",lang ? "Chercher:" : "Search:", search,
                      false);
  ...
  while (1) {
    ...
    if ((expr = Console_GetLine()) == NULL)
      ...
      if (confirm(lang ? "Effacer variables?" : "Clear variables?",
                  lang ? "F1: annul,  F5: confirmer" : "F1: cancel,  F5: confirm") != KEY_CTRL_F5) {
