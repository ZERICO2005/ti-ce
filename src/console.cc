...
char menu_f1[maxfmenusize] = {0}, menu_f2[maxfmenusize] = {0}, menu_f3[maxfmenusize] = {0}, menu_f4[maxfmenusize] = {0}, menu_f5[
       maxfmenusize] = {0}, menu_f6[maxfmenusize], menu_f7[maxfmenusize] = {0}, menu_f8[maxfmenusize] = {0}, menu_f9[maxfmenusize] = {0},
     menu_f10[maxfmenusize] = {0}, menu_f11[maxfmenusize] = {0}, menu_f12[maxfmenusize] = {0}, menu_f13[maxfmenusize] = {0}, menu_f14[
       maxfmenusize] = {0}, menu_f15[maxfmenusize] = {0}, menu_f16[maxfmenusize] = {0}, menu_f17[maxfmenusize] = {0}, menu_f18[
       maxfmenusize] = {0}, menu_f19[maxfmenusize] = {0}, menu_f20[maxfmenusize] = {0};
char session_filename[MAX_FILENAME_SIZE + 1] = "session";
char * FMenu_entries_name[] = {
  menu_f1, menu_f2, menu_f3, menu_f4, menu_f5, menu_f6, menu_f7, menu_f8, menu_f9, menu_f10, menu_f11, menu_f12, menu_f13, menu_f14,
  menu_f15, menu_f16, menu_f17, menu_f18, menu_f19, menu_f20
};
...
void draw_filled_arc(int x, int y, int rx, int ry, int color, int theta1_deg, int theta2_deg, int xmin, int xmax, int ymin, int ymax,
                     bool segment) {
  //chk_freeze();
...
void draw_line(int x1, int y1, int x2, int y2, int color, unsigned short motif) {
  ...
  ++w;
  color &= 0xffff;
  if ((x1 < 0 && x2 < 0) || (x1 >= LCD_WIDTH_PX && x2 >= LCD_WIDTH_PX) || (y1 < clip_ymin && y2 < clip_ymin) || (y1 >= LCD_HEIGHT_PX && y2
    >= LCD_HEIGHT_PX))
    return;
  signed char ix;
  signed char iy;
  ...
  if (delta_x >= delta_y) {
    ...
    if (lcdptr) {
      ...
      while (x1 != x2) {
        if (error >= 0) {
          if (error || (ix > 0)) {
            if (iy == 1) lcdptr += LCD_WIDTH_PX;
            else lcdptr -= LCD_WIDTH_PX;
            y1 += iy;
            error -= delta_x;
          } // else do nothing
        } // else do nothing
...
int inputline(const char * msg1, const char * msg2,std::string & s, bool numeric, int ypos) {
  ...
  for (;;) {
    ...
    if (beg > pos)
      beg = pos;
    textX = X1;
    int cursorpos = os_draw_string_medium(textX, textY,SDK_BLACK,SDK_WHITE, s.substr(beg, pos - beg).c_str(),false);
    // PrintMini(&textX,&textY,(Char *)s.substr(beg,pos-beg).c_str(),0x02, 0xFFFFFFFF, 0, 0, COLOR_BLACK, COLOR_WHITE, 1, 0); int cursorpos=textX;
    os_draw_string_medium(cursorpos, textY,SDK_BLACK,SDK_WHITE, s.substr(pos, s.size() - pos).c_str(),false);
    // PrintMini(&textX,&textY,(Char*)s.substr(pos,s.size()-pos).c_str(),0x02, 0xFFFFFFFF, 0, 0, COLOR_BLACK, COLOR_WHITE, 1, 0);
...
void chk_clearscreen() {
  ...
  if (confirm(lang ? "Effacer l'historique?" : "Clear history?",lang ? "F1: annuler,   F5: effacer" : "F1: cancel,   F5: erase",
              true) == KEY_CTRL_F5) {
    Console_Init();
    Console_Clear_EditLine();
  }
...
void console_disp_status(int keyflag) {
  Console_FMenu_Init();
  string menu(" "), shiftmenu = menu, alphamenu;
  int menucolorbg = 12345;
        ...
        if (sres == MENU_RETURN_SELECTION) {
          ...
          if (smallmenu.selection == 3) {
            ...
            if (fileBrowser(filename, (char*)"*.xw", (char*)"Sessions")) {
              if (console_changed == 0 || strcmp(session_filename, "session") == 0 || confirm(
                lang ? "Session courante perdue?" : "Current session will be lost",
                lang ? "F1: annul, F5: ok" : "F1: cancel, F5: ok") == KEY_CTRL_F5) {
...
const char * console_menu(int key, Char * cfg_, int active_app) {
  ...
  char temp[maxentry_size], menu1[maxentry_size], menu2[maxentry_size], menu3[maxentry_size], menu4[maxentry_size], menu5[maxentry_size],
       menu6[maxentry_size], menu7[maxentry_size], menu8[maxentry_size], menu9[maxentry_size];
...
bool tooltip(int x, int y, int pos, const char * editline) {
  ...
  if (l1 > 0
    ...
    // display tooltip
    if (x < 0)
      x = os_draw_string(0, y, _BLACK, 1234, editline,true);
    // fake print -> x position // replaced cmdline by editline so that tooltip is at end
    x += 2;
    y += 4;
    ...
    if (linecomment) {
      ...
      if (*src == '"') {
        ...
        if (i == 0) {
          ...
          for (; *src; ++i, ++src) {
            if (*src == ' ' || (i && *src >= ' ' && *src <= '/') || (python && *src == '#') || (!python && *src == '/' && *(src
              + 1) == '/') || *src == '"' || isalpha(*src))
              break;
          }
        }
