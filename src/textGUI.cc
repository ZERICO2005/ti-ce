...
int end_do_then(const std::string & s) {
  ...
  if (i > 0) {
    ...
    if (strcmp(ptr, "faire") == 0 || strcmp(ptr, "do") == 0 || strcmp(ptr, "alors") == 0 || strcmp(ptr, "then") == 0)
      return 1;
    if (strcmp(ptr, "fsi") == 0 || strcmp(ptr, "end") == 0 || strcmp(ptr, "fi") == 0 || strcmp(ptr, "od") == 0 ||
      strcmp(ptr, "ftantque") == 0 || strcmp(ptr, "fpour") == 0 || strcmp(ptr, "ffonction") == 0 || strcmp(ptr, "ffunction") == 0)
      return -1;
  }
  return 0;
}
...
void chk_restart() {
  ...
  if (confirm(lang ? "Conserver les variables?" : "Keep variables?",
              lang ? "F1: conserver,   F5: effacer" : "F1: keep,   F5: erase") == KEY_CTRL_F5)
    do_restart();
}
...
std::string get_selection(textArea * text, bool erase) {
  ...
  if (erase) {
    ...
    text->line = sel_line1;
    text->pos = sel_pos1;
    text->elements[sel_line1].s = s.substr(0, sel_pos1) + text->elements[sel_line2].s.substr(
      sel_pos2, text->elements[sel_line2].s.size() - sel_pos2);
  }
...
void warn_python(int mode, bool autochange) {
  if (mode == 0)
    confirm(autochange
              ? (lang ? "Source en syntaxe Xcas detecte." : "Xcas syntax source code detected.")
              : (lang ? "Syntaxe Xcas." : "Xcas syntax."), "F1/F5: ok");
  if (mode == 1) {
    if (autochange)
      confirm(lang ? "Passage en syntaxe Python." : "Setting Python syntax source.",
              lang ? "avec ^=**, F1/F5: ok" : "with ^=**, F1/F5:ok");
    else
      confirm(lang ? "Syntaxe Python avec ^==**," : "Python syntax with ^==**,",
              lang ? "python_compat(2): xor. F1: ok" : "python_compat(2): xor. F1: ok");
  }
  if (mode == 2) {
    confirm(lang ? "Syntaxe Python avec ^==xor" : "Python syntax with ^==xor",
            lang ? "python_compat(1): **. F1: ok" : "python_compat(1): **. F1: ok");
  }
}
...
void textarea_disp(textArea * text, int & isFirstDraw, int & totalTextY, int & scroll, int & textY, bool minimini) {
  ...
  for (int cur = 0; cur < v.size(); ++cur) {
    ...
    if (cur == 0) {
      ...
      if (l >= 8 && src[0] == 'f' && (src[1] == 'o' || src[1] == 'u') && src[2] == 'n' && src[3] == 'c' && src[4] == 't' && src[5] ==
        'i' && src[6] == 'o' && src[7] == 'n')
        change_mode(text, 0); // text->python=false;
    ...
    if (editable) {
      ...
      textX = os_draw_string_small(textX, textY,TEXT_COLOR_PURPLE,TEXT_COLOR_WHITE, line_s,/*fake*/!do_printline);
      // PrintMiniMini(&textX, &textY, (Char *)line_s, 0, TEXT_COLOR_PURPLE, 0 );
    }
    textX = text->x + deltax;
    ...
    while (*src) {
      ...
      if (linecomment || !text->editable)
        ...
        if (*src == '"') {
          ...
          if (i == 0) {
            ...
            for (; i < 35 && *src; ++i, ++src) {
              if (*src == ' ' || (i && *src == ',') || (text->python && *src == '#') || (!text->python && *src == '/' && *(src
                + 1) == '/') || *src == '"' || isalpha(*src))
                break;
            }
          }
      ...
      if (temptextX < text->width && temptextX + textX > text->width - 6) {
        if (editable)
          os_draw_string_medium(textX, textY,COLOR_MAGENTA,COLOR_WHITE, ">",/* fake*/!do_printline);
        //PrintMini(&textX, &textY, (Char*)"\xe6\x9b", 0x02, 0xFFFFFFFF, 0, 0, COLOR_MAGENTA, COLOR_WHITE, 1, 0);
        //time for a new line
        textX = text->x + deltax;
        textY = textY + text->lineHeight + v[cur].lineSpacing;
        ++nlines;
      ...
      if (textY >= -C24 && textY < LCD_HEIGHT_PX) {
        ...
        if (editable) {
          ...
          if (!mini_) {
            // translate colors, not the same for printingg
        ...
        if (editable && cur == textline) {
          if (oldsrc <= srcpos && (srcpos < src || (srcpos == src && textpos == tlen))) {
            if (textpos >= 2 && v[cur].s[textpos - 1] == ' ' && v[cur].s[textpos - 2] != ' ' && srcpos - oldsrc == strlen(
              singleword) + 1) {
...
bool move_to_word(textArea * text, const std::string & s, const std::string & replace, int & isFirstDraw, int & totalTextY, int & scroll,
                  int & textY) {
  if (!s.size())
    return false;
...
int handle_key(textArea * text, int key, int keyflag, int & isFirstDraw, int & scroll, int & textY, int & totalTextY,std::string & search,
               std::string & replace) {
  //dbg_printf("key=%i\n",key);
  std::vector<textElement> & v = text->elements;
  bool editable = text->editable;
    ...
    if (clipline < 0) {
      ...
      if (adds) {
        ...
        if (1 || text->python) {
          ...
          if (isif)
            adds = lang
                     ? (isex ? "si x<0 alors x:=-x; fsi;" : "si  alors\n\nsinon\n\nfsi;")
                     : (isex ? "if x<0 then x:=-x; fi;" : "if  then\n\nelse\n\nfi;");
          if (lang && iselse)
            adds = "sinon ";
          if (isfor)
            adds = lang
                     ? (isex ? "pour j de 1 jusque 10 faire\nprint(j*j);\nfpour;" : "pour  de  jusque  faire\n\nfpour;")
                     : (isex ? "for j from 1 to 10 do\nprint(j*j);\nod;" : "for  from  to  do\n\nod;");
          if (isforin)
            adds = lang
                     ? (isex ? "pour j in [1,4,9,16] faire\nprint(j)\nfpour;" : "pour  in  faire\n\nfpour;")
                     : (isex ? "for j in [1,4,9,16] do\nprint(j);od;" : "for  in  do\n\nod;");
          if (iswhile)
            adds = lang
                     ? (isex
                          ? "a,b:=25,15;\ntantque b!=0 faire\na,b:=b,irem(a,b);\nftantque;a;"
  ...
  case KEY_CTRL_F5:
    if (clipline < 0 && text->editable && text->filename.size()) {
      ...
      if (sres == MENU_RETURN_SELECTION) {
        ...
        if (sres == 7) {
          textarea_disp(text, isFirstDraw, totalTextY, scroll, textY, text->minimini);
          int l = get_line_number(lang ? "Negatif: en partant de la fin" : "Negative: counted from the end",
                                  lang ? "Numero de ligne:" : "Line number:");
          if (l > 0)
            text->line = l - 1;
          if (l < 0)
            text->line = v.size() + l;
...
int doTextArea(textArea * text) {
  ...
  while (1) {
    ...
    if (key == KEY_CTRL_SYMB)
      key = KEY_CTRL_F4;
    if (!text->editable && ((key >= KEY_CTRL_F1 && key <= KEY_CTRL_F4) || key == KEY_CTRL_F6 || key == KEY_CTRL_F7 || key == KEY_CTRL_F8
      || (key >= KEY_CTRL_F11 && key <= KEY_CTRL_F13)))
      return key;
